  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-12 max-w-2xl">
        ...
        {articles.length === 0 ? (
          <p className="text-muted-foreground">No bookmarks yet. Save articles from the blog to see them here.</p>
        ) : (
              ...
              <Card key={a.id} className="flex flex-row items-center justify-between p-3 gap-2">
                <Link href={`/blog/${a.slug}`} className="flex-1 min-w-0">
                  <CardHeader className="p-0 cursor-pointer">
                    <CardTitle className="text-base font-medium hover:text-primary truncate">{a.title}</CardTitle>
                  </CardHeader>
                </Link>
